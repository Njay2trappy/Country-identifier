...
const REST_COUNTRIES_ENDPOINT = 'https://restcountries.com/v2/all?fields=name,capital,region,population,flag,currencies,independent';
...
export class CountryService {
    ...
    async getCountryByName(name) {
        ...
        const country = this.countryStore.find((item) => item.name?.toLowerCase() === name.trim().toLowerCase());
        if (!country) {
            throw new ServiceError('NOT_FOUND', `Country not found: ${name}`);
        }
        return this.toCountryResponse(country);
    }
    getCountries({ filters, sort } = {}) {
        if (this.countryStore.length === 0) {
            throw new ServiceError('NOT_FOUND', 'No countries stored in database');
        }
    ...
    getCountriesConnection(args = {}) {
        ...
        const hasPreviousPage = windowStart > 0 || truncatedByLast || windowIndices[0] > windowStart;
        const hasNextPage = windowEnd < sortedRecords.length || truncatedByFirst ||
    ...
    async storeCountries() {
        try {
            ...
            this.nextId = 1;
            const rawCountries = await this.httpGetJson(REST_COUNTRIES_ENDPOINT);
            this.countryStore = rawCountries.map((country) => this.buildCountryRecord(country));
            await this.persist();
            return {
                success: true,
                message: `Stored ${this.countryStore.length} countries`,
                count: this.countryStore.length,
                countries: this.countryStore.map((country) => this.toCountryResponse(country)),
            };
        }
        catch (error) {
            return this.handleOperationFailure('Error storing countries', error, { count: 0, countries: [] });
        }
    }
    async addExchangeRates() {
        try {
            if (this.countryStore.length === 0) {
                throw new ServiceError('VALIDATION', 'No countries stored. Run `storeCountries` first.');
            }
            this.lastRefreshedAt = new Date().toISOString();
            const exchangeRateData = await this.httpGetJson(EXCHANGE_RATE_API_ENDPOINT);
            ...
            return this.handleOperationFailure('Error adding exchange rates', error, { count: 0, countries: [] });
        }
    }
    async deleteCountry(name) {
        try {
            ...
            const index = this.countryStore.findIndex((country) => country.name?.toLowerCase() === name.trim().toLowerCase());
            if (index === -1) {
                throw new ServiceError('NOT_FOUND', `Country not found: ${name}`);
            }
            ...
            return this.handleOperationFailure('Error deleting country', error, { deletedCountry: null });
        }
    }
    async generateCountryImage() {
        try {
            if (this.countryStore.length === 0) {
                throw new ServiceError('VALIDATION', 'No countries stored. Run `storeCountries` and `addExchangeRates` first.');
            }
            ...
            if (top5ByGdp.length === 0) {
                throw new ServiceError('VALIDATION', 'Exchange rate data not available. Refresh exchange rates first.');
            }
            ...
            return this.handleOperationFailure('Error generating summary image', error, { imagePath: null });
        }
    }
    applyFilters(records, filters) {
        ...
        return records.filter((country) => {
            ...
            const matchesCurrency = currency
                ? country.currencies.some((curr) => curr.code?.toLowerCase() === currency.toLowerCase())
                : true;
    ...
    buildCountryRecord(rawCountry, id) {
        ...
        return {
            ...
            currencies,
            independent: typeof rawCountry.independent === 'boolean'
    ...
    async ensureFetch() {
        ...
        if (!this.cachedFetch) {
            ...
            this.cachedFetch = fetchImpl;
        }
        return this.cachedFetch;
    }
    async httpGetJson(endpoint) {
        try {
            const fetchImpl = await this.ensureFetch();
            const response = await fetchImpl(endpoint);
            if (!response.ok) {
                throw new ServiceError('EXTERNAL_API', `Failed to fetch ${endpoint}: HTTP ${response.status}`);
            }
            ...
            throw new ServiceError('EXTERNAL_API', error instanceof Error ? error.message : 'Unknown network error');
        }
    }
    async buildSummaryImageBuffer({ top5ByGdp, totalCountries, lastRefreshedAt, }) {
        const fetchImpl = await this.ensureFetch();
                ...
                scales: {
                    ...
                    y: {
                        ticks: {
                            color: '#2d3748',
                            font: { size: 14 },
                            callback: 'function(value) { return Intl.NumberFormat("en-US", { notation: "compact", maximumFractionDigits: 1 }).format(value); }',
                        },
                        beginAtZero: true,
                    },
                },
    ...
    handleOperationFailure(context, error, defaults) {
        if (error instanceof ServiceError) {
            return {
                ...defaults,
                success: false,
